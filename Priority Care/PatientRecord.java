...
//
// Title:    This class models PatientRecord objects to be managed in a priority
...
/**
 * This class models PatientRecord objects to be managed in a priority queue at an urgent
 * care service.
 */
public class PatientRecord implements Comparable<PatientRecord> {
    ...
    // Begins at 1, and advances to next value after each caseID is generated.
    ...
    /**
     * For tester class purposes only: resets PatientRecord.patientCounter to 1.
     ...
     * @return {@code true} if other is instanceof PatientRecord and this PatientRecord
     * and other have
    ...
    public int compareTo(PatientRecord other) {
        if (this.triage == other.triage) {//checks if the triage are similar
            return this.orderOfArrival - other.orderOfArrival;
        }
        return this.triage.compareTo(other.triage);//returns the expected value.
    }
}
